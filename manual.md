...
1. **Applicability Engine (`applicabilityEngine.js`)**: Evaluates the endpoint context against the `checklist.json` to filter out non-applicable checks. This is **batched per endpoint**—a single LLM call evaluates the endpoint against the entire checklist simultaneously. The result is then cached (in-memory, and persistently via `--cache`) by endpoint signature (`METHOD /path`), ensuring each endpoint is only processed once per run.
2. **Probe Synthesizer (`probeSynthesizer.js`)**: For every checklist item requiring active testing on a given endpoint, the AI synthesizes an intelligent, context-aware HTTP request tailored specifically for the vulnerability being tested — **also batched per endpoint**, one call covering every still-undecided item at once (items already resolved by a prior run's `--cache` file are skipped entirely, never re-sent). Note that probe generation operates with a non-zero temperature (`0.1`) when not using `--cache`, meaning **probes can slightly drift between runs** on the same target. This provides fuzzing-like variance but means runs are not strictly deterministic.
3. **Verdict Classifier (`verdictClassifier.js`)**: Evaluates the HTTP response from every synthesized probe on an endpoint that survived the deterministic 404/401/403 gates — **also batched per endpoint**, one call classifying every surviving probe at once. The AI assigns a status (`PASS`, `FAIL`, `N/A`, `TO BE CONFIRMED`) and a confidence score per probe. If the AI's confidence falls below defined thresholds (e.g. `AI_FAIL_CONFIDENCE_THRESHOLD`), the tool will gracefully downgrade the verdict to `TO BE CONFIRMED` to prevent false positives. Verdicts are never cached — each depends on a live HTTP response that can change between runs — so this stage still runs in full on every scan, which is why batching it matters most.

Batching keeps each stage at roughly one call per endpoint instead of one call per checklist item — for a checklist with N applicable items requiring active testing, that's ~3 AI calls per endpoint total instead of up to 1 + 2N. A hard per-call cap (`MAX_BATCH_SIZE` in each module) keeps any single prompt bounded, splitting into multiple calls only if an endpoint has more undecided items than the cap.

For checks in the checklist that can be covered by hardcoded scripts (e.g., standard SQLi fuzzing), the engine routes them back to the traditional deterministic modules (e.g. `injection/sqliXss`) — no AI call at all.
